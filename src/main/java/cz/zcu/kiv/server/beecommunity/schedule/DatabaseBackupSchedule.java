...
public class DatabaseBackupSchedule {
    ...
    private static final String DATABASE_CONTAINER_NAME = "beecommunity_postgres_1";
    ...
    public void scheduleDatabaseBackup() {
        ...
        if (!successExitCode) {
            ...
            successExitCode = runSingleCommand(
                    "docker",
                    "cp",
                    String.format("%s:%s", DATABASE_CONTAINER_NAME, backupName),
                    outputFile
            );
            // Check return code of copy command
            if (!successExitCode) {
                log.error("Unable to move backup {} from container {}", backupName, DATABASE_CONTAINER_NAME);
